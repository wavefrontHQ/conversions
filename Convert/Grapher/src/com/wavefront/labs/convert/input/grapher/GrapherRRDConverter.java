...
import com.wavefront.labs.convert.input.rrd.RRDConverter;
import com.wavefront.labs.convert.input.rrd.models.Graph;
import com.wavefront.labs.convert.input.rrd.models.graph.Area;
...
import com.wavefront.labs.convert.input.rrd.models.graph.Stack;
...
public class GrapherRRDConverter extends RRDConverter {
	...
	public List convert() {
		...
		List<String> notifications = Arrays.asList(properties.getProperty("grapher.alert.notifications", "test@alert.com").split(" "));
		...
		for (Graph graph : rrdContext.getGraphs()) {
			...
			if (graph.getGraphItemType() == GraphItemType.AREA && graphItem.getColor().startsWith("COLORST")) {
				...
				alert.setNotificants(notifications);
